...
from sqlalchemy import Column, Integer, String, ForeignKey, Float, Enum, DateTime, Boolean, Text
...
class Project(Base):
    ...
    budget = Column(Float, nullable=True)  # Optional budget for the project
    ...
    invoice = relationship("Invoice", back_populates="project")
    pending_approval = relationship(
        "PendingApproval",
        primaryjoin="and_(PendingApproval.entity == 'project', foreign(PendingApproval.entity_id) == Project.id)",
        viewonly=True
    )
